...
pub const Color = struct {
    ...
    pub fn fromRGBf(r: f32, g: f32, b: f32) Color {
        return Color{
            .r = @intCast(r * 255.0),
            .g = @intCast(g * 255.0),
            .b = @intCast(b * 255.0),
...
pub fn set(x: usize, y: usize, color: Color) !void {
    ...
    switch (bpp) {
        ...
        BPP_32 => {
            ...
            // Usually memory bytes become B, G, R, 0 on little-endian x86.
            p.* = color.as_bpp32();
        },
...
pub fn fill(color: Color) !void {
    ...
    switch (bpp) {
        ...
        BPP_32 => {
            const color_value = color.as_bpp32();

            if (pitch % 4 != 0) return error.InvalidPitch;

            const words = (pitch * height) / 4;
            const ptr: [*]volatile u32 = @ptrFromInt(fb_addr);
            const slice: []volatile u32 = ptr[0..words];

            @memset(slice, color_value);
        },
        else => return error.UnsupportedBPP,
    }
}
